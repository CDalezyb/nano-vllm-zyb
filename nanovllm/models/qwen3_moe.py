...
from nanovllm.layers.fuse_moe import (
    MergedColumnParallelFusedMoeLinear,
    RowParallelFusedMoeLinear,
    get_expert_counts_and_idx,
)
...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ''' This is never used, maybe just for perf comparision '''
    def __init__(
        self,
        num_experts: int,
        hidden_size: int,
        ...
        self.experts = Qwen3MoeExperts(
            num_experts, hidden_size, intermediate_size, hidden_act
        )
...
class Qwen3MoeFusedSparseMoeBlock(nn.Module):
    def __init__(
        ...
        self.gate = RowParallelLinear(hidden_size, num_experts, bias=False)
        self.gate_up_proj = MergedColumnParallelFusedMoeLinear(
            hidden_size, [intermediate_size] * 2, num_experts
        )
        self.down_proj = RowParallelFusedMoeLinear(
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # decoder_sparse_step 表示每隔几层一个稀疏MoE层
        ...
        for file in glob(os.path.join(path, "*.safetensors")):
            with safe_open(file, "pt", "cpu") as f:
                for weight_name in f.keys():
                    ...
                    is_loaded = False
                    # print(f"[debug]: weight_name: {weight_name}, is_expert: {is_expert}")
